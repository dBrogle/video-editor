...
def calculate_centered_geometry(
    image_path: Path, safe_zone: dict, align_bottom: bool = False
) -> str:
    """
    ...
    Args:
        ...
        align_bottom: If True, bottom-align the image within the safe zone
            instead of vertically centering it (keeps horizontal centering).
    ...
    if align_bottom:
        y_offset = safe_zone["bottom"] - fitted_height
    else:
...
def add_zoom_filter(
    ...
    """
    Add an affine zoom/pan filter to a chain element.

    Args:
        chain: Chain ET.Element to add the filter to
        filter_id: Unique filter ID (e.g., "filter0")
        video_width: Video width in pixels (e.g., 1080)
        video_height: Video height in pixels (e.g., 1920)
        zoom_factor: Zoom level (1.0 = no zoom, 1.5 = 50% zoom in)
        x_offset: Horizontal pan (-1 to 1, 0 = centered)
        y_offset: Vertical pan (-1 to 1, 0 = centered)
    """
    ...
    # Apply pan offset (scaled to the available pan range)
    ...
    filt = ET.SubElement(chain, "filter", {"id": filter_id})
...
def convert_gif_to_video(gif_path: Path, loop_duration: float = 30.0) -> Path:
    """
    Convert an animated GIF to a looping MP4 video for use in MLT.

    Args:
        gif_path: Path to the GIF file
        loop_duration: Target duration in seconds (GIF will loop to fill this)
    ...
    # Use ffmpeg to convert GIF to MP4, looping to fill the target duration
    # -stream_loop -1 loops the input, -t limits the output duration
    cmd = [
        "ffmpeg", "-y",
        "-stream_loop", "-1",
        "-i", str(gif_path),
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Failed to convert GIF to video: {result.stderr}")
    return output_path
...
def add_image_producer(
    ...
    Supports animated GIFs by converting them to looping MP4 videos.
    ...
    if is_gif or is_video:
        ...
        ET.SubElement(producer, "property", {"name": "resource"}).text = str(image_path)
        ET.SubElement(producer, "property", {"name": "ttl"}).text = "1"
        ET.SubElement(producer, "property", {"name": "aspect_ratio"}).text = "1"
        ET.SubElement(producer, "property", {"name": "progressive"}).text = "1"
        ET.SubElement(producer, "property", {"name": "seekable"}).text = "1"
        ET.SubElement(producer, "property", {"name": "format"}).text = "1"
...
def create_main_tractor(
    ...
    Args:
        ...
        num_overlay_tracks: Number of image overlay tracks (playlist1, playlist2, ...)
    """
    ...
    # Add tracks: background (0), video/playlist0 (1), overlay playlists (2, 3, ...)
    ET.SubElement(tractor, "track", {"producer": "background"})
    ET.SubElement(tractor, "track", {"producer": "playlist0"})
    for i in range(num_overlay_tracks):
        ET.SubElement(tractor, "track", {"producer": f"playlist{i + 1}"})

    # Transitions for background <-> video
    add_mix_transition(tractor, "transition0", 0, 1)
    add_cairo_transition(tractor, "transition1", 0, 1)

    # Transitions for each overlay track
    transition_idx = 2
    for i in range(num_overlay_tracks):

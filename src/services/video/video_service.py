...
class VideoService:
    ...
    def create_final_cut(
        ...
        """
        Create final cut video from original high-resolution video using adjusted sentences.
        This applies the same edits from the downsampled workflow to the original video.

        NOTE: The pipeline now uses MLTVideoService for this step. This method is kept
        as a fallback option using ffmpeg filter_complex.

        Uses ffmpeg directly with filter_complex for efficient handling of large files.
        This approach avoids loading the entire video into memory like MoviePy does.

        Args:
            base_name: Base filename without extension
            adjusted_sentences: AdjustedSentences with silence-trimmed timestamps
            force: If True, regenerate even if file exists
            preset: ffmpeg encoding preset (ultrafast, veryfast, faster, fast, medium, slow, slower, veryslow)
                   Default 'faster' provides good speed/quality balance
        ...
        print_progress(f"Creating final cut from high-res video: {input_path.name}...")

        if not adjusted_sentences.sentences:
            raise ValueError("No sentences provided - cannot create video")

        print_progress(
            f"Applying edits from {len(adjusted_sentences.sentences)} adjusted sentences"
        )

        try:
            # Build ffmpeg filter complex for trimming and concatenating
            segments = []
            for i, sentence in enumerate(adjusted_sentences.sentences):
                start = sentence.adjusted_start
                end = sentence.adjusted_end
                duration = end - start

                segments.append({"start": start, "duration": duration, "index": i})

            print_progress(f"Building ffmpeg filter for {len(segments)} segments...")

            # Create filter complex for all segments
            # Each segment needs both video and audio trimming
            filter_parts = []
            for seg in segments:
                idx = seg["index"]
                # Format with 2 decimal places for timestamps
                filter_parts.append(
                    f"[0:v]trim=start={seg['start']:.2f}:duration={seg['duration']:.2f},setpts=PTS-STARTPTS[v{idx}];"
                    f"[0:a]atrim=start={seg['start']:.2f}:duration={seg['duration']:.2f},asetpts=PTS-STARTPTS[a{idx}]"
                )

            # Build concatenation part
            v_streams = "".join(f"[v{i}][a{i}]" for i in range(len(segments)))
            filter_complex = (
                ";".join(filter_parts)
                + f";{v_streams}concat=n={len(segments)}:v=1:a=1[outv][outa]"
            )
            ...
            print_progress("Running ffmpeg to create final cut...")
            print_progress(f"  Using preset '{preset}' for encoding")
            ...
            if result.returncode != 0:
                print_progress(f"ffmpeg STDERR: {result.stderr}")
                raise RuntimeError(
                    f"ffmpeg failed with return code {result.returncode}: {result.stderr}"
                )

            print_progress(f"Final cut created: {output_path.name}")
            tf = time.time()
            print_progress(f"Final cut created in {tf - ti:.2f} seconds")
            return output_path

        except Exception as e:
            raise RuntimeError(f"Final cut creation failed: {str(e)}") from e

    def extract_final_cut_audio(
        self,
        base_name: str,
        sample_rate: int = AUDIO_SAMPLE_RATE,
        channels: int = AUDIO_CHANNELS,
        ...
        """
        Extract audio from final cut video as mono WAV file.
        ...
        Raises:
            FileNotFoundError: If final cut doesn't exist
            RuntimeError: If ffmpeg fails
        """
        input_path = get_final_cut_path(base_name)
        validate_file_exists(input_path)

        # Build output path
        output_path = get_final_cut_audio_path(base_name)

        # Skip if exists and not forcing
        if output_path.exists() and not force:
            print_progress(f"Final cut audio already exists: {output_path}")
            return output_path

        print_progress(
            f"Extracting audio from final cut ({sample_rate}Hz, {channels}ch)..."
        )
        ...
        try:
            subprocess.run(cmd, capture_output=True, text=True, check=True)
            print_progress(f"Final cut audio extracted: {output_path}")
            return output_path
    ...
    def downsample_final_cut(
        self, base_name: str, height: int = LOW_RES_HEIGHT, force: bool = False
    ) -> Path:
        """
        Downsample the final cut video to low resolution.
        ...
        Returns:
            Path to downsampled final cut video
        ...
        # Build output path
        output_path = get_final_cut_downsampled_path(base_name)

        # Skip if exists and not forcing
        if output_path.exists() and not force:
            print_progress(f"Downsampled final cut already exists: {output_path}")
            return output_path

        print_progress(f"Downsampling final cut to {height}p...")
        ...
        try:
            subprocess.run(cmd, capture_output=True, text=True, check=True)
            print_progress(f"Downsampled final cut created: {output_path}")
            return output_path

        except subprocess.CalledProcessError as e:
            raise RuntimeError(
                f"ffmpeg downsampling failed:\n"
                f"Command: {' '.join(cmd)}\n"
                f"Exit code: {e.returncode}\n"
                f"stderr: {e.stderr}"
            ) from e
